...
template <typename Dtype>
void AccuracyLayer<Dtype>::Forward_cpu(const vector<DataBlock<Dtype>*>& bottom,
    const vector<DataBlock<Dtype>*>& top) {
  Dtype accuracy = 0;
  const Dtype* bottom_data = bottom[0]->GetCpuData();
  const Dtype* bottom_label = bottom[1]->GetCpuData();
  const int dim = bottom[0]->count() / outer_num_;
  const int num_labels = bottom[0]->shape(label_axis_);
  if (top.size() > 1) {
    bsy_set(nums_buffer_.count(), Dtype(0), nums_buffer_.GetCpuData());
    bsy_set(top[1]->count(), Dtype(0), top[1]->GetCpuData());
  }
  int count = 0;
  for (int i = 0; i < outer_num_; ++i) {
    for (int j = 0; j < inner_num_; ++j) {
      ...
      if (top.size() > 1) ++nums_buffer_.GetCpuData()[label_value];
      ...
      if (num_better_predictions < top_k_) {
        ++accuracy;
        if (top.size() > 1) ++top[1]->GetCpuData()[label_value];
      }
      ++count;
    }
  }
  ...
  top[0]->GetCpuData()[0] = (count == 0) ? 0 : (accuracy / count);
  if (top.size() > 1) {
    for (int i = 0; i < top[1]->count(); ++i) {
      top[1]->GetCpuData()[i] =
          nums_buffer_.GetCpuData()[i] == 0 ? 0
          : top[1]->GetCpuData()[i] / nums_buffer_.GetCpuData()[i];
    }
  }
