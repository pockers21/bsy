...
void PoolingLayer<Dtype>::LayerSetUp(const vector<DataBlock<Dtype>*>& bottom,
...
template <typename Dtype>
void PoolingLayer<Dtype>::Reshape(const vector<DataBlock<Dtype>*>& bottom,
      const vector<DataBlock<Dtype>*>& top) {
...
// case?
template <typename Dtype>
void PoolingLayer<Dtype>::ForwardCpu(const vector<DataBlock<Dtype>*>& bottom,
      const vector<DataBlock<Dtype>*>& top) {
  ...
  case PoolingParameter_PoolMethod_MAX:
    // Initialize
    if (use_top_mask) {
      top_mask = top[1]->GetCpuData();
      bsy_set(top_count, Dtype(-1), top_mask);
    } else {
      mask = max_idx_.GetCpuData();
      bsy_set(top_count, -1, mask);
    }
    bsy_set(top_count, Dtype(-FLT_MAX), top_data);
    // The main loop
...
template <typename Dtype>
void PoolingLayer<Dtype>::BackwardCpu(const vector<DataBlock<Dtype>*>& top,
  ...
  bsy_set(bottom[0]->count(), Dtype(0), bottom_diff);
  // We'll output the mask to top[1] if it's of size >1.
  const bool use_top_mask = top.size() > 1;
