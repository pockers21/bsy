#ifndef bsy_SOFTMAX_WITH_LOSS_LAYER_HPP_
#define bsy_SOFTMAX_WITH_LOSS_LAYER_HPP_
 ...
 *
 * @param bottom input DataBlock vector (length 2)
 *   -# @f$ (N \times C \times H \times W) @f$
 *      the predictions @f$ x @f$, a DataBlock with values in
 ...
 *      the labels @f$ l @f$, an integer-valued DataBlock with values
...
class SoftmaxWithLossLayer : public LossLayer<Dtype> {
 ...
 protected:
   ...
   *   -# @f$ (1 \times 1 \times 1 \times 1) @f$
   *      This DataBlock's diff will simply contain the loss_weight* @f$ \lambda @f$,
   *      as @f$ \lambda @f$ is the coefficient of this layer's output
   *      @f$\ell_i@f$ in the overall Net loss
   *      @f$ E = \lambda_i \ell_i + \mbox{other loss terms}@f$; hence
   *      @f$ \frac{\partial E}{\partial \ell_i} = \lambda_i @f$.
   *      (*Assuming that this top DataBlock is not used as a bottom (input) by any
  ...
  /// on the DataBlock size.  If normalization_mode is VALID, the count of valid
  ...
  DataBlock<Dtype> prob_;
  /// bottom vector holder used in call to the underlying SoftmaxLayer::Forward
  vector<DataBlock<Dtype>*> softmax_bottom_vec_;
  /// top vector holder used in call to the underlying SoftmaxLayer::Forward
  vector<DataBlock<Dtype>*> softmax_top_vec_;
...
#endif  // bsy_SOFTMAX_WITH_LOSS_LAYER_HPP_
